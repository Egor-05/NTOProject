...
def go(aktsii, risk, res_pars):
    res_pars = filter(aktsii, res_pars)
    m_o = math_o(res_pars)
    cov = cov_table(res_pars)
    return port(cov, m_o, risk)


def get_min_profit(max_risk, companies):
    rez = ShareInfo.objects.filter(name__in=companies).all()
    print(rez)
    return go([], max_risk, rez)


def get_max_risk(min_profit):
    pass